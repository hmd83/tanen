...
- Custom GATT service: 16 characteristics (creds, calibration, timing, thresholds, commands, live sensors, Extended Mode config + live BLE sensors, load-cell profile)
...
- Load-cell profile: **LcProfile** `0x0010` (R/W, 10 B = `lc_config_t`, ZMS id 21 — version, profile id, gain mg/K, τ s). Picks which cell the temperature correction runs with (generic, H40A-C3-0150, SBS-PF-150, Zemic L6E, TAL220, Flintec PC/SB, or own measured values), so one image serves every frame. Only the H40A is characterised; the other named cells carry gain 0 and behave like generic until measured, but the unit records which one is fitted and a later firmware fills the constants in — non-custom profiles are resolved from the table on every read, not from the stored copy. A **read returns the effective constants**, resolved from the profile table, which is why the web page needs no copy of it; a write honours gain/τ only for `LC_PROFILE_CUSTOM` and is range-checked (ATT `0x13` otherwise). The write calls `tempcomp_reload()` so the 2 s live view already shows the new correction. The tare survives a profile change — the correction is zero at `T_ref` for any gain. Profile ids are the wire contract: append, never reorder (`docs/load_cells/README.md`)
